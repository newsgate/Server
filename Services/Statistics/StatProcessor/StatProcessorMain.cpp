...
/**
 * @file   NewsGate/Server/Services/Statistics/StatProcessor/StatProcessorMain.cpp
...
namespace
{
  ...
  const char USAGE[] = "Usage: StatProcessor <config_file> <orb options>";
}

StatProcessorApp::StatProcessorApp() throw (El::Exception)
    : logger_(new El::Logging::StreamLogger(std::cerr))
{
}

void
StatProcessorApp::terminate_process() throw(El::Exception)
{
  if(stat_processor_impl_.in() != 0)
  {
    stat_processor_impl_->stop();
    stat_processor_impl_->wait();
  }
}

void
StatProcessorApp::main(int& argc, char** argv) throw()
{
  try
  {
    try
    {
      ...
      if (argc < 2)
      {
        ...
        ostr << "StatProcessorApp::main: config file is not specified\n"
             << USAGE;
      ...
      {
        throw Exception("StatProcessorApp::main: Unable to resolve RootPOA.");
      }
      ...
      // Creating message stat_processor server servant
      
      stat_processor_impl_ = new NewsGate::Statistics::StatProcessorImpl(this);
    
      // Creating Object Ids

      PortableServer::ObjectId_var stat_processor_obj_id =
        PortableServer::string_to_ObjectId("StatProcessor");
      ...
      service_poa->activate_object_with_id(stat_processor_obj_id,
                                           stat_processor_impl_.in());
      
      service_poa->activate_object_with_id(process_control_id, this);
      
      // Registering reference in IOR table

      CORBA::Object_var stat_processor =
        service_poa->id_to_reference(stat_processor_obj_id);
      ...
      orb_adapter->add_binding("StatProcessor",
                               stat_processor,
                               "PersistentIdPOA");

      stat_processor_impl_->start();
    ...
    {
      std::ostringstream ostr;
      ostr << "StatProcessorApp::main: CORBA::Exception caught. "
        "Description: \n" << e;
    ...
    {
      std::ostringstream ostr;
      ostr << "StatProcessorApp::main: xml_schema::exception caught. "
        "Description: \n" << e;
    ...
    {
      std::ostringstream ostr;
      ostr << "StatProcessorApp::main: El::Exception caught. Description: \n"
           << e;
  ...
  catch (...)
  {
    Application::logger()->emergency(
      "StatProcessorApp::main: unknown exception caught",
      ASPECT);
  }
}

bool
StatProcessorApp::notify(El::Service::Event* event) throw(El::Exception)
{
