...
/**
 * @file   NewsGate/Server/Services/Moderator/CustomerManager/CustomerManagerMain.cpp
...
namespace
{
  ...
  const char USAGE[] = "Usage: CustomerManager <config_file> <orb options>";
}

CustomerManagerApp::CustomerManagerApp() throw (El::Exception)
    : logger_(new El::Logging::StreamLogger(std::cerr))
{
}

void
CustomerManagerApp::terminate_process() throw(El::Exception)
{
  if(customer_manager_impl_.in() != 0)
  {
    customer_manager_impl_->stop();
    customer_manager_impl_->wait();
  }
}

void
CustomerManagerApp::main(int& argc, char** argv) throw()
{
  try
  {
    try
    {
      ...
      if (argc < 2)
      {
        ...
        ostr << "CustomerManagerApp::main: config file is not specified\n"
             << USAGE;
      ...
      {
        throw Exception("CustomerManagerApp::main: "
      ...
      // Creating customer manager servant
      
      customer_manager_impl_ =
        new NewsGate::Moderation::CustomerManagerImpl(this);
    
      // Creating Object Ids

      PortableServer::ObjectId_var customer_manager_obj_id =
        PortableServer::string_to_ObjectId("CustomerManager");
      ...
      service_poa->activate_object_with_id(customer_manager_obj_id,
                                           customer_manager_impl_.in());
      
      service_poa->activate_object_with_id(process_control_id, this);
      
      // Registering reference in IOR table

      CORBA::Object_var customer_manager =
        service_poa->id_to_reference(customer_manager_obj_id);
      ...
      orb_adapter->add_binding("CustomerManager",
                               customer_manager,
                               "PersistentIdPOA");

      customer_manager_impl_->start();
    ...
    {
      std::ostringstream ostr;
      ostr << "CustomerManagerApp::main: CORBA::Exception caught. "
        "Description: \n" << e;
    ...
    {
      std::ostringstream ostr;
      ostr << "CustomerManagerApp::main: xml_schema::exception caught. "
        "Description: \n" << e;
    ...
    {
      std::ostringstream ostr;
      ostr << "CustomerManagerApp::main: El::Exception caught. Description: \n"
           << e;
  ...
  catch (...)
  {
    Application::logger()->emergency(
      "CustomerManagerApp::main: unknown exception caught",
      ASPECT);
  }
}

bool
CustomerManagerApp::notify(El::Service::Event* event) throw(El::Exception)
{
